    ...
    const fetchQuestion = async () => {
      try {
        const res = await axios.get(
          ...
          `${API_URL}/students/getQuize/${practiceId}`
        );
    ...
    const timer = setInterval(() => {
      setRemainingTime((prevTime) => {
        if (prevTime > 0) {
          ...
          navigation.navigate("student/(screens)/Exams/practiceTests");
          return prevTime;
        }
      });
    }, 1000);
      ...
      <ScrollView showsVerticalScrollIndicator={false}>
        ...
        >
          {question.questions && question.questions.length > 0 && (
            ...
            >
              <Text className="text-sm text-text font-normal font-primary mb-2">
                Question
              </Text>
              <LinearGradient
                colors={colors.gr1}
              ...
              {question.questions[currentQuestion]?.question && (
                <Text className="text-[18px] text-mainBlack/80 mt-2 font-bold">
                  {question.questions[currentQuestion].question}
                </Text>
              )}
                    ...
                    style={
                      selectedOptions.find(
                        ...
                        ? {
                            backgroundColor:
                              option.answer === true
                                ? "#0BDA5110" // If the option is correct, set the background to green
                                : colors.red + "10", // If the option is incorrect, set the background to red
                            padding: 16,
                            borderRadius: 8,
                            borderColor:
                              option.answer === true ? "#0BDA51" : colors.red, // If the option is correct, set the border color to green
                            borderWidth: 1,
                          }
                        : {
                  ...
                  >
                    <RadioIndicator mr="$2">
                      <RadioIcon
                        ...
                        style={{
                          color:
                            option.answer === true ? "#0BDA51" : colors.red,
                          borderColor:
                            option.answer === true ? "#0BDA51" : colors.red,
                        }}
                      />
                      ...
                      style={
                        selectedOptions.find(
                          ...
                          ? {
                              fontFamily: "Urbanist",
                              color:
                                option.answer === true ? "#0BDA51" : colors.red, // Change text color to green if option is correct
                              fontSize: 16,
                              marginLeft: 10,
                              fontWeight:
                                option.answer === true ? "bold" : "bold", // Make font bold for correct options
                            }
                          : {
                              fontFamily: "Urbanist",
                              color: colors.text,
