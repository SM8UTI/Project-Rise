...
const login = () => {
  ...
  const handleSubmit = async () => {
    ...
    await axios
      .post(`${API_URL}/teacher/teacherLogin`, {
      ...
      .then((res) => {
        ...
        if (res.data.success === true) {
          AsyncStorage.setItem("teacherData", JSON.stringify(res.data.teacher));
          router.push("teacher/chooseClass");
          AsyncStorage.setItem("teacherId", res.data.teacher._id);
        }
  ...
  return (
    <SafeAreaView className="bg-white pt-0 w-full h-full flex items-center justify-center">
      ...
      <View className="flex flex-col items-center justify-center gap-2 mt-6">
        <Text
          className="text-3xl text-black  font-primary font-bold"
          style={{
            fontFamily: "Urbanist",
          }}
        >
    ...
    //   {/* <Link href="teacher/home" asChild>
    //     <Button title="Open teacher Home Screen" />
    //   </Link> */}
    // </View>
  );
};
