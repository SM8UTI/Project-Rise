...
const PracticeExamCard = ({ userId, data }) => {
  const subjectIcon = {
  ...
  const { _id, name, scorePerQuestion, questions, subject, totalTime } = data;
  ...
  return (
    <TouchableOpacity
      ...
      onPress={() => router.push(`student/(screens)/practiceExam/${_id}`)}
    >
      <HStack space="lg">
        <Box
          bg="transparent"
        ...
        <VStack space="sm">
          <Box bg="#fff">
            ...
            <Text className="text-sm text-text font-primary">{subject}</Text>
          </Box>
          <HStack
            space="md"
            style={{
